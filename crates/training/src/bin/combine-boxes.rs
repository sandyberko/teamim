...
fn main() -> eyre::Result<()> {
    ...
    let files = if args.inputs.contains(&InputType::Synthetic) {
        ...
        let outs = fs::read_dir(assets_dir.join("corrected_boxfiles"))?
            .map(|entry| {
                let entry = entry?;
...
fn generate_lstmf_cmd(output_path_stem: &Path, tif_path: PathBuf, psm: PageSegMode) -> Command {
