    ...
    for c in text.chars() {
        ...
        if let Some(g) = font.outline_glyph(glyph) {
            ...
            bounds = Some(if let Some(bounds) = bounds {
                // union
                Rect {
                    min: point(bounds.min.x.min(bb.min.x), bounds.min.y.min(bb.min.y)),
                    max: point(bounds.max.x.max(bb.max.x), bounds.max.y.max(bb.max.y)),
                }
            } else {
                bb
            });
...
pub fn text_size(scale: impl Into<PxScale> + Copy, font: &impl Font, text: &str) -> (Rect, f32) {
